...
countValid x = length (filter checkValid x)

checkValid :: String -> Bool
checkValid str = length str >= 6 && (str !! 2 == str !! 4 || str !! 3 == str !! 5)
...
sumSuccess list = case sumSucces' list of [] -> Left "no data"
                                          a  -> Right (sum a)

sumSucces' :: [Either String Int] -> [Int]
sumSucces' []     = []
sumSucces' (x:xs) = case x of Left _    -> sumSucces' xs
                              Right num -> num : sumSucces' xs
...
changeCode str (Open _) = Open str
changeCode _ x          = x
...
compose [] _ = []
compose ((x, y) : xs) ys = case lookup y ys of Just z  -> (x, z) : compose xs ys
...
permute idx list = permute' 0 idx list

permute' :: Int -> Permutation -> [a] -> [a]
permute' n idx list = case elemIndex n idx of
  Just a -> (list !! a) : permute' (n + 1) idx list
  Nothing -> []